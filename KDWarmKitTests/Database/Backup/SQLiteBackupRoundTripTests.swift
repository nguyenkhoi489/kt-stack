...
final class SQLiteBackupRoundTripTests: XCTestCase {
    ...
    override func setUp() async throws {
        ...
        workDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("kdwarm-sqlite-backup-\(UUID().uuidString)", isDirectory: true)
    ...
    func testBackupAndRestoreOverwriteRoundTrip() async throws {
        ...
        let profile = ConnectionProfile(name: "test", kind: .sqlite, host: "", port: 0,
                                        user: "", database: "main", filePath: dbPath, readOnly: false)
    ...
    func testRestoreNewDatabaseDoesNotOverwriteExistingFile() async throws {
        ...
        do {
            try await provider.restore(profile: profile, password: nil,
                                        from: snapshot, into: .newDatabase(target.path))
