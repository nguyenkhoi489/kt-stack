...
public struct SQLiteBackupProvider: BackupProvider {
    ...
    public var fileExtension: String { "sqlite" }
    ...
    private static func overwriteFile(at targetPath: String, with snapshot: URL) throws {
        ...
        let safetyDir = directory.appendingPathComponent(".kdwarm-restore-\(UUID().uuidString)",
                                                         isDirectory: true)
        ...
        let staged = directory.appendingPathComponent(".kdwarm-staged-\(UUID().uuidString).sqlite")
