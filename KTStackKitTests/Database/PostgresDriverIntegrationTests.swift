...
/// Opt-in integration coverage for `PostgresDriver` against the managed engine. Gated on `KTSTACK_DB_IT=1`
...
final class PostgresDriverIntegrationTests: XCTestCase {
    ...
    func testReadOnlyConnectionRejectsWrites() async throws {
        ...
        do {
            ...
            _ = try? await driver.query("DROP TABLE IF EXISTS ktstack_ro_guard", database: nil)
