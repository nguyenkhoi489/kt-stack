...
enum MySQLCellMapper {
    ...
    static func cell(
        definition column: MySQLProtocol.ColumnDefinition41,
    ...
    static func mysqlData(for cell: Cell) -> MySQLData {
        switch cell {
        case let .text(s): return MySQLData(string: s)
        case let .int(n): return MySQLData(int: Int(n))
        case let .double(d): return MySQLData(double: d)
        case let .bool(b): return MySQLData(bool: b)
